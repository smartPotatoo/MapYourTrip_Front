          ...
          <div className='time-memo-list-container'>
            {props.item.times.length !== 0 ?
              props.item.times.map((item, index) => (
                <div key={index} className="time-item-container">
                  <ScheduleTimeItem item={item} />
                  {(props.type === 'create' || props.type === 'modify') && (
                    <button className="remove-time-button" onClick={() => props.onRemoveTime(props.item.date, index)}>
                      삭제
                    </button>
                  )}
                </div>
